        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    sprintf(distanceStr, "C: %.2f m", distance);

                    for (int i = 0; distanceStr[i]; i++)
                    {
                      while (app_uart_put(distanceStr[i]) != NRF_SUCCESS);
                    }

                    /*uint8_t cr = '0' + (uint8_t)distance;
                    //while (app_uart_get(&cr) != NRF_SUCCESS);
                    while (app_uart_put(distance) != NRF_SUCCESS);
